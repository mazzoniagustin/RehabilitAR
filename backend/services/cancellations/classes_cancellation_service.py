...
def cancelar_clase(class_id, cancel_reason):
    try:
        if (cancel_reason == None) or (cancel_reason == ""):
            ...
            supabase.table("classes").update({"status": "cancelada", "cancellation_reason": cancel_reason}).eq("id", class_id).execute()
    ...
    try:
        if (class_id != None):
            ...
            if (clase["professor_id"] is None) and (diferencia_horas <= 12):
                supabase.table("classes").update({"status": "cancelada", "cancellation_reason": "Cancelación automática por falta de profesor"}).eq("id", class_id).execute()
                reservas = supabase.table("reservations").select("*").eq("class_id", class_id).eq("status", "activa").execute()
                for reserva in reservas.data:
                    supabase.table("reservations").update({"status": "cancelada"}).eq("id", reserva["id"]).execute()
                    #otorgar_credito(reserva["user_id"], reserva["class_id"]) #pendiente_de_implementar
