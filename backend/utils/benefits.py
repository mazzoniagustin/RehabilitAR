...
def retirar_Todoscredito(user_id: str):
    """
    FIX: ya no lanza excepción si el usuario tiene 0 créditos.
    Se llama como efecto secundario al alcanzar el límite de cancelaciones;
    si no hay créditos simplemente no hay nada que retirar.
    """
    try:
        resultado = supabase.table("credits").select("available_credits").eq("user_id", user_id).execute()
        if not resultado.data:
            return {"message": "Sin créditos que retirar."}

        credito = resultado.data[0]
        if credito["available_credits"] <= 0:
            return {"message": "Sin créditos que retirar."}
